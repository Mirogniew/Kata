 ...
 * to walk (eg. ['n', 's', 'w', 'e']). You always walk only a single block for each letter (direction) and you
...
public class CartesiaWalk {
    public static boolean isValid(char[] walk) {
        ...
        int[] distance = {0, 0};
        for (char c : walk) {
            distanceChecker(c, distance);
        }
        return distance[0] == 0 && distance[1] == 0;
    }

    public static void distanceChecker(char walk, int[] distance) {
        switch (walk) {
            case 'n' -> distance[0]++;
            case 's' -> distance[0]--;
            case 'e' -> distance[1]++;
